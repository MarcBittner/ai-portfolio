#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for agent-sandbox (replaces make).
...
cmd_demo()  {
  ensure_installed
  py -c "from agent_sandbox.agent import run; r=run('What is 20% of the days between 2026-01-01 and 2026-02-01?'); [print(f'step {i+1}: {s.tool}({s.args}) -> {s.observation}') for i,s in enumerate(r.steps)]; print('answer:', r.answer)"
}
