...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline paved-road demo (scaffold → onboard → ingest)
...
cmd_demo()  { ensure_installed; py -m baseplate.demo; }
cmd_eval()  { ensure_installed; py -m baseplate.evaluate "$@"; }
