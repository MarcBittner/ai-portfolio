"""Offline demo of the standard platform setup.

Scaffold a new service from a free-text description, show the generated
Terraform/k8s/CI/service-health files, add it to the catalog, then run the
example price-transparency ingest workload and show its data-quality score.
...
def main() -> None:
    ...
    print("Self-service scaffolder — free text → service spec → standard platform files")
    ...
    print(f"   extracted service spec: name={spec.name} language={spec.language} "
    ...
    print(f"   generated {len(gen['files'])} standard platform files:")
    ...
    print("\n   CI/CD pipeline head (.github/workflows/ci.yml):")
    print(_preview(gen["files"][".github/workflows/ci.yml"], 10))

    print("\n   Service-health targets (slo.yaml):")
    ...
    print("\nExample workload: price-transparency ingest + data-quality score")
    dq = ingest.score()
    print(f"   loaded {dq['rows']} synthetic rate rows → "
          f"{dq['valid']} valid / {dq['invalid']} invalid")
    print(f"   data-quality pass rate = {dq['data_quality_pass_rate']}  "
          f"defects={dq['defects']}")
    for f in dq["sample_failures"]:
        print(f"     row {f['row']}: {', '.join(f['issues'])}")

    print("\nService-health targets (availability + latency + data-quality):")
