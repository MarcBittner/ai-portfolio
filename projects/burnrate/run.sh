#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for burnrate (replaces make).
...
Commands:
  ...
  serve       Run the Flask API + UI (gunicorn)
  test        Run the test suite (skips opt-in live smoke)
  lint        Run ruff
  check       lint + test
  demo        Run the offline burn → incident-summary → recover demo
  eval        Run the reproducible eval (writes eval-report.md)
  smoke       Run the live smoke/regression suite (local server, or --url <deploy>)
  doctor      Report Python / venv / Redis / Ollama status
  help        Show this help
...
cmd_doctor() {
  ...
  if redis-cli -u "${REDIS_URL:-redis://localhost:6379}" ping >/dev/null 2>&1; then
    grn "redis: reachable (background tasks defer via TaskTiger-shaped queue)"
  else
    dim "redis: not reachable (background tasks run inline — capability still works)"
  fi
  if curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    ...
    dim "ollama: not reachable (incident summary falls back to the deterministic drafter)"
  fi
}
...
# No --url: start a local gunicorn, wait for /healthz, run, tear down.
...
cmd_smoke() {
  ...
  if [[ -n "$url" ]]; then
    ...
    py -m gunicorn "$APP" --bind "127.0.0.1:$PORT" --workers 1 >"$log" 2>&1 &
    pid=$!
    local up=0 i
    for i in $(seq 1 60); do
      if py -c "import urllib.request; urllib.request.urlopen('$url/healthz', timeout=2)" 2>/dev/null; then up=1; break; fi
...
cmd_serve() { ensure_installed; tool gunicorn "$APP" --bind "$HOST:$PORT" --workers 1 --timeout 60; }
cmd_test()  { ensure_installed; py -m pytest -q -k "not live_smoke" "$@"; }
cmd_lint()  { ensure_installed; tool ruff check src tests; }
cmd_demo()  { ensure_installed; py -m burnrate.demo; }
cmd_eval()  { ensure_installed; py -m burnrate.evaluate "$@"; }
