#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for counsel (replaces make).
...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline grounded-copilot + trust-gate demo
  eval        Run the grounding/verify/guardrail/trust-gate eval → eval-report.md
...
Env:
  ...
  LLM_MODE     Routing mode (auto|paid|local|free|offline); offline needs no keys
EOF
}
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (local LLM narration available)"
  else
    dim "ollama: not reachable (narration falls back to the deterministic offline responder)"
  fi
}
...
# Run the live smoke suite against a real HTTP endpoint (local or remote).
cmd_smoke() {
  ensure_installed
...
cmd_demo()  { ensure_installed; py -m counsel.demo; }
cmd_eval()  { ensure_installed; py -m counsel.evaluate "$@"; }
