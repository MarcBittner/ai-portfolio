#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for llm-gateway (replaces make).
...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline governed-pipeline demo
  eval        Score the firewall on the labeled set → eval-report.md
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (local routing available)"
  else
    dim "ollama: not reachable (routing falls back to the deterministic mock)"
  fi
}
...
cmd_demo()  { ensure_installed; py -m llm_gateway.demo; }
cmd_eval()  { ensure_installed; py -m llm_gateway.evaluate; }
