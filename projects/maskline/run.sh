#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for maskline (replaces make).
...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline governance walkthrough
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (local LLM classification available)"
  else
    dim "ollama: not reachable (classification falls back to the deterministic detector)"
  fi
}
...
cmd_demo()  { ensure_installed; py -m maskline.demo; }
cmd_eval()  { ensure_installed; py -m maskline.evaluate "$@"; }
