...
- **Free-text columns** (a ``CLAIM_NOTE``) are where PHI hides in prose that no
  column rule sees. maskline samples a few values and asks the **LLM** (via the
  portfolio routing chain, JSON mode) which PHI types are embedded. The
  deterministic ``offline`` fallback is regex + name heuristics over the same
  samples and returns the **same JSON shape**, so classification works (and the
  eval reproduces) with zero keys.
...
from maskline import llm, warehouse
...
def classify_column(table: str, col: dict, *, mode: str | None = None) -> dict:
    ...
    if cls is None:  # free text → ask the model
        samples = col.get("samples") or warehouse.sample_values(table, name)
        user = "Column: {}\nSamples:\n{}".format(name, "\n".join(samples))
