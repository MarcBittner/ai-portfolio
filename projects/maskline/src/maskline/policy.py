...
- **Per column-class → a masking function.** ``direct`` is fully masked,
  ``quasi`` is generalized (ZIP→3 digits, dates→year), ``clinical``/``financial``
  are visible to analysts, ``non_sensitive`` is untouched. The generated
  ``CREATE MASKING POLICY`` is column-data-type-aware and keyed off the caller's
  Snowflake role (``CURRENT_ROLE()``).
- **Per role → a row predicate.** A row-access policy scopes which rows a role
  sees (e.g. an analyst sees only de-identified/non-flagged rows).

``generate_snowflake_ddl()`` emits the real artifact (``CREATE MASKING POLICY`` /
``CREATE ROW ACCESS POLICY`` / ``ALTER TABLE … SET …``). ``generate_terraform()``
emits the same intent as ``snowflake_masking_policy`` / ``snowflake_row_access_policy``
resources. ``coverage()`` reports which discovered sensitive columns have **no**
masking policy — the governance gap.
"""
...
# Roles that may see unmasked sensitive data (the privileged analytics roles are
# NOT among them — masking applies to everyone except these).
UNMASKED_ROLES = ("PHI_STEWARD", "COMPLIANCE_AUDITOR")

# Per column-class → (policy name, masked return expression). The expression is a
# Snowflake SQL fragment over the masked column value ``val``.
...
def _masked_expr(cls: str, arg_type: str) -> str:
    ...
    if arg_type != "VARCHAR":
        ...
        return "NULL" if cls == "quasi" else "NULL"
    return expr
...
def required_policies(classified: list[dict]) -> dict[str, dict]:
    ...
    Keyed by ``(policy_name, arg_type)`` so a class that appears over both VARCHAR
    and NUMBER columns yields one policy per argument type (Snowflake requires the
    policy signature to match the column type).
    """
    ...
    for c in classified:
        ...
        if cls not in CLASS_MASK:
            continue
        # Structured masking policies bind to a named column. A free-text column
        # whose PHI was discovered by the LLM (method == "llm") is NOT covered by
        # the name-driven policy set — it surfaces as a coverage gap / gate
...
def generate_snowflake_ddl(classified: list[dict]) -> str:
    ...
    out: list[str] = [
        "-- maskline: generated Snowflake masking + row-access policy-as-code",
        f"-- target: {warehouse.FQ}",
    ...
    for p in policies.values():
        for tbl, colname in p["columns"]:
            out.append(
                f"ALTER TABLE {warehouse.FQ}.{tbl} "
...
def generate_terraform(classified: list[dict]) -> str:
    ...
    policies = required_policies(classified)
    out: list[str] = [
        "# maskline: generated Terraform (snowflake provider) for masking +",
...
def coverage(classified: list[dict]) -> dict:
    ...
    A column is covered iff its class is either in ``CLASS_MASK`` (a policy is
    generated for it) AND that class requires masking. Clinical/financial are
    sensitive-but-intentionally-visible, so they are not gaps; only ``direct`` /
    ``quasi`` columns that are missing a policy fail the gate.
    """
    policies = required_policies(classified)
