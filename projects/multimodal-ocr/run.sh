#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for multimodal-ocr (replaces make).
# Production-grade: strict mode, dependency + version checks, --flag options.
set -euo pipefail

# ---- project config ----
PROJECT="multimodal-ocr"
PKG="multimodal_ocr"
APP="multimodal_ocr.api:app"
MIN_PY_MAJOR=3
MIN_PY_MINOR=11
DEFAULT_PORT=8008
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (local LLM routing available)"
  else
    dim "ollama: not reachable (LLM features fall back to the deterministic mock)"
  fi
}
...
cmd_demo()  {
  ensure_installed
  py -c "from multimodal_ocr.ocr import sample_tokens; from multimodal_ocr.pipeline import process; r=process(sample_tokens('receipt')); print('redacted:', r.redacted_text.replace(chr(10),' | ')); print('boxes:', len(r.boxes), 'counts:', r.counts)"
}
