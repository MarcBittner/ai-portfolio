"""Structural detectors over internet-intelligence host records → exposure findings.

Each detector reads explicit fields the scanner already parsed (open port, exposure
class, software + version, TLS certificate metadata) and, when it fires, emits a
finding with a stable ``rule_id``, a severity, the affected ``asset`` (host:port),
machine-readable ``evidence``, the controls it affects, and a remediation. The
finding carries its control ids at the moment of detection, so the mapping is never
an afterthought — the report is governed evidence, not a raw scanner dump.
...
from perimeter.data import EOL_SOFTWARE, SCAN_DATE
...
# TLS versions considered deprecated for data-in-transit.
_DEPRECATED_TLS = {"1.0", "1.1"}
# Signature algorithms considered weak.
_WEAK_SIG = {"sha1withrsa", "md5withrsa"}
# Minimum acceptable RSA key size (bits). ECDSA keys are sized differently and are
# accepted at >= 256.
_MIN_RSA_BITS = 2048
# Certs within this many days of expiry are flagged as expiring-soon.
...
def derive_service(host: dict, svc: dict, as_of: date) -> list[dict]:
    """All findings for one observed service on one host."""
