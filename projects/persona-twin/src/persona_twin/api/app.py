...
class ChatRequest(BaseModel):
    ...
    # Browser→host bridge: prose generated by the user's own Ollama in the
    # browser (against the prompt from /chat/prepare). When present, the server
    # uses it as the answer instead of calling a provider; routing is recorded
    # as "ollama (browser→host)". Default None keeps the normal path intact.
    client_completion: str | None = Field(default=None, max_length=20000)
    client_model: str | None = Field(default=None, max_length=200)


class ChatPrepareResponse(BaseModel):
    """The local tier's inputs, built server-side without a model call."""
...
@router.post("/chat/prepare", response_model=ChatPrepareResponse)
async def chat_prepare(request: ChatRequest) -> ChatPrepareResponse:
    """Browser→host step 1: build the local tier's prompt without a model call.

    The cloud server can't reach a user's ``localhost:11434`` — only the
    browser can. So the browser asks the server to prepare: the server runs the
    same retrieval as ``/chat`` and returns the exact SYSTEM + user prompt (plus
    the retrieved chunk ids), the browser runs it on the host's Ollama, then
    posts the completion back to ``/chat`` as ``client_completion``.

    Prepare does NOT append the message to session history — only the finalizing
    ``/chat`` call does — so a prepare followed by a real chat does not
    double-count the turn. The ``session_id`` is echoed (generated when omitted)
    so the browser threads the follow-up correctly."""
    new_request_id()
    state = _state()
    ...
    history = state.sessions.history(session_id)
    ...
    REQUESTS.inc("chat", "prepared")
    return ChatPrepareResponse(session_id=session_id, **prepared.to_payload())
    ...
    async def events():
        ...
        try:
            async for ev in chat_twin(
                ...
                k=request.k,
                condense=state.settings.chat_condense,
                client_completion=request.client_completion,
