#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for quorum (replaces make).
...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline multi-agent contract-review + replicability demo
  eval        Run the contract-review eval → eval-report.md
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (local agent routing available)"
  else
    dim "ollama: not reachable (agents fall back to the deterministic offline path)"
  fi
}
...
cmd_demo()  { ensure_installed; py -m quorum.demo; }
cmd_eval()  { ensure_installed; py -m quorum.evaluate "$@"; }
