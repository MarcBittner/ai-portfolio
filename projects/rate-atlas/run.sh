...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline normalize + compare demo
  eval        Run the column-mapping + invariants eval → eval-report.md
...
cmd_demo()  { ensure_installed; py -m rate_atlas.demo; }
cmd_eval()  { ensure_installed; py -m rate_atlas.evaluate "$@"; }
