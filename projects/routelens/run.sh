...
Commands:
  ...
  serve       Run the proxy + console (uvicorn, --reload)
  test        Run the test suite
  lint        Run ruff
  check       lint + test
  demo        Run the offline demo: generate traffic, observe, route, report
  eval        Run the reproducible eval → eval-report.md
  smoke       Run the live smoke/regression suite (local server, or --url <deploy>)
  doctor      Report Python / venv / model-provider status
  help        Show this help
...
cmd_doctor() {
  ...
  if command -v ollama >/dev/null 2>&1 || curl -sf -m 1 "${OLLAMA_BASE_URL:-http://localhost:11434}/api/tags" >/dev/null 2>&1; then
    grn "ollama: reachable (real shadow-judging available against local models)"
  else
    dim "ollama: not reachable — set a provider key or start Ollama to measure quality"
  fi
}
