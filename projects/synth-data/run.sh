#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for synth-data (replaces make).
...
cmd_demo()  {
  ensure_installed
  py -c "from synth_data.generate import generate, PRESETS, to_csv; print(to_csv(generate(PRESETS['users'], 5, seed=42)))"
}
