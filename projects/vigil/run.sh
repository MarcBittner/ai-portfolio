#!/usr/bin/env bash
# run.sh — dev/ops entrypoint for vigil (replaces make).
...
Commands:
  ...
  lint        Run ruff
  check       lint + test
  demo        Run the offline fleet-monitoring demo (synthetic probes + LLM summary)
...
cmd_demo()  { ensure_installed; py -m vigil.demo; }
