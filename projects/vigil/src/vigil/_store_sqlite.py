...
This is the **default backend** behind ``vigil.store`` (selected when ``MONGODB_URI``
is unset — i.e. every local + test run). The parallel Mongo backend in
``_store_mongo.py`` implements the identical function surface; ``store.py`` is a
thin selector between the two, so every other vigil module imports ``from vigil
import store`` and stays backend-agnostic.
...
**Opaque string ids.** SQLite assigns integer ``rowid`` autoincrement ids, but
every id this module returns is rendered as a ``str`` (``str(rowid)``) and every
``get/update/delete``-by-id accepts the string form (coerced back to int for the
query). This mirrors the Mongo backend's ``str(ObjectId)`` so api.py and the SPA
treat ids as opaque strings regardless of which backend is active.
...
def _coerce_id(value: object) -> int | None:
    """Accept an opaque string id (or int) and return the integer rowid, or None if
    it isn't a valid integer id. The public API hands us ``str`` ids (to match the
    Mongo backend's ``str(ObjectId)``); we coerce back to int for the SQLite query.
    A non-numeric id (e.g. a Mongo ObjectId hex passed to the wrong backend) yields
    None, which the callers turn into a clean 'not found' rather than a crash."""
    try:
        return int(value)
...
def _strid(d: dict, *keys: str) -> dict:
    """In-place: render the named integer-id columns of a row dict as strings (or
    leave None as None). Keeps ids opaque + backend-agnostic across the API/UI."""
    for k in keys:
...
def delete_check(check_id: str | int) -> bool:
    ...
    if cid is None:
        return False
    with _conn() as c:
        cur = c.execute("DELETE FROM checks WHERE id=?", (cid,))
        c.execute("DELETE FROM check_results WHERE check_id=?", (cid,))
        return cur.rowcount > 0
    ...
    with _conn() as c:
        c.execute(
            """INSERT INTO check_results (check_id, slug, ts, passed, http_status,
               ...
               VALUES (?,?,?,?,?,?,?,?)""",
            (cid, slug, time.time(), int(passed), http_status, response_ms,
        ...
        c.execute(
            ...
            (cid, cid, config.MAX_HISTORY_PER_TARGET),
        )
...
def get_user_by_email(email: str) -> dict | None:
    ...
    return _strid(dict(r), "id") if r else None
...
def get_user_by_id(uid: str | int) -> dict | None:
    ...
    if iid is None:
        return None
    with _conn() as c:
        r = c.execute("SELECT * FROM users WHERE id=?", (iid,)).fetchone()
    return _strid(dict(r), "id") if r else None
...
def list_users() -> list[dict]:
    ...
    return [_strid(dict(r), "id") for r in rows]
...
def record_alert_event(rule_id: str | int | None, slug: str, channel: str,
    ...
    rid = _coerce_id(rule_id) if rule_id is not None else None
    with _conn() as c:
        c.execute(
            """INSERT INTO alert_events (rule_id, slug, ts, channel, message, delivered)
               VALUES (?,?,?,?,?,?)""",
            (rid, slug, time.time(), channel, message, int(delivered)),
        )
...
def recent_alert_events(limit: int = 50) -> list[dict]:
    ...
    return [_strid(dict(r), "id", "rule_id") for r in rows]
